...
The keras model is created by training [SmallerVGGNet](model.png) from scratch on around 2200 face images (~1100 for each class). Face region is cropped by applying `face detection` using `cvlib` on the images gathered from Google Images. It acheived around 96% training accuracy and ~90% validation accuracy.
...
When you run the script for the first time, it will download the pre-trained model and place it under `pre-trained` directory in the current path.
